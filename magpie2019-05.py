...
class Intersection(NamedTuple):
    ...
    def make_pattern_generator(clue: Clue, intersections: Sequence['Intersection'], clue_list: ClueList) -> \
        ...
        pattern_list = ['[1-9]' if location in clue_list.start_locations else '.' for location in clue.locations()]
        pattern_list.append('$')
...
class Solver:
    ...
    def __solve(self, current_index: int) -> None:
        ...
        current_letter_values = set(self.known_letters.values())
        pattern = solving_step.pattern_maker(self.known_clues)
        try:
            for next_letter_values in itertools.permutations(range(1, 17), len(clue_letters)):
                if any(v in current_letter_values for v in next_letter_values):
                    continue
