...
def test_all_precedence_interactions2():
    parse = _parse1(" -√a! ** b' * +cg + d / e - f ")
    assert parse.expression == (
        '-', ('+', ('*', ('-', ('√', ('**', ('!', 'a'), ("'", 'b')))), ('+', ('*', 'c', 'g'))),
               ('/', 'd', 'e')), 'f')
    assert parse.to_string() == '(((-sqrt(fact(a) ** prime(b))) * (+(c * g))) + (d / e)) - f'
    assert parse.to_string(concise=True) == '-sqrt(fact(a) ** prime(b)) * +(c * g) + d / e - f'
    assert parse.to_string(concise=True, pure=True) == '(-√a! ** b\') (+c g) + d / e - f'
...
def test_pure_simple_mul():
    assert _parse1('a * b').to_string(concise=True, pure=True) == 'a b'

def test_pure_triple_mul():
    assert _parse1('a * b * c').to_string(concise=True, pure=True) == 'a b c'

def test_pure_mul_add_left():
    assert _parse1('(a + b) * c').to_string(concise=True, pure=True) == '(a + b) c'

def test_pure_mul_add_right():
    assert _parse1('a * (b + c)').to_string(concise=True, pure=True) == 'a (b + c)'

def test_pure_mul_both_add():
    assert _parse1('(a + b) * (c + d)').to_string(concise=True, pure=True) == '(a + b) (c + d)'

def test_pure_add_of_muls():
    assert _parse1('a * b + c * d').to_string(concise=True, pure=True) == 'a b + c d'

def test_pure_division_not_juxtaposed():
    assert _parse1('a / b').to_string(concise=True, pure=True) == 'a / b'

def test_pure_factorial():
    assert _parse1('a!').to_string(concise=True, pure=True) == 'a!'
...
def test_pure_factorial_mul():
    assert _parse1('a! * b').to_string(concise=True, pure=True) == 'a! b'

def test_pure_power_mul():
    assert _parse1('a ** b * c').to_string(concise=True, pure=True) == 'a ** b c'

def test_pure_mul_power():
    assert _parse1('a * b ** c').to_string(concise=True, pure=True) == 'a b ** c'
