import itertools
from typing import Tuple, Sequence, Dict, List, Set
...
class Sudoku:
    ...
    def solve(self, puzzle: str, *,
        ...
        def row_printer(results: Sequence[Tuple[int, int, int]]) -> None:
            self.draw_grid(results)

        self.constraints = self.__get_default_constraints()
        ...
        self.handle_chess(king, knight, adjacent)
            ...
            for (index1, (row1, col1)), (index2, (row2, col2)) in itertools.combinations(enumerate(thermometer), 2):
                for value1, value2 in itertools.product(range(1, 10), repeat=2):
                    if value2 < value1 + (index2 - index1):
                        self.not_both((row1, col1, value1), (row2, col2, value2), prefix)

        if magic:
            self.handle_magic(deletions)
                ...
                for ((row1, col1), (row2, col2)) in zip(cup, cup[1:]):
                    for value1, value2 in itertools.product(range(1, 10), repeat=2):
                        if abs(value1 - value2) < 5:
                            self.impossible((row1, col1, value1), (row2, col2, value2))
            ...
            for ((row1, col1), (row2, col2)) in zip(square, square[1:] + square[:1]):
                for value1, value2 in itertools.product(range(1, 10), repeat=2):
                    if value1 + value2 not in (1, 4, 8, 9, 16):
                        self.impossible((row1, col1, value1), (row2, col2, value2), 'c')

            def row_printer(results: Sequence[Tuple[int, int, int]]) -> None:
                values_in_circle = {value for (row, column, value) in results if (row, column) in square}
                if len(values_in_circle) == 9:
                    self.draw_grid(results)
        ...
        for i, snake in enumerate(snakes):
            for value in range(1, 10):
                ...
                for row, column in snake:
                    self.constraints[row, column, value].append(constraint)
        ...
        links = DancingLinks(self.constraints, optional_constraints=self.optional_constraints, row_printer=row_printer)
        links.solve(debug=1000, recursive=False,)

    def handle_chess(self, king, knight, adjacent):
            ...
            if knight or king:
                ...
                for neighbor_row, neighbor_column in neighbors:
                    if (row, column) < (neighbor_row, neighbor_column):
                        for value in range(1, 10):
                            self.not_both((row, column, value), (neighbor_row, neighbor_column, value), 'C')
            ...
            if adjacent:
                ...
                for neighbor_row, neighbor_column in self.adjacent_move(row, column):
                    for value in range(1, 9):  # Note, we don't need to use value=9.
                        self.not_both((row, column, value), (neighbor_row, neighbor_column, value + 1))

    def handle_magic(self, deletions):
    ...
    def not_both(self, triple1: Tuple[int, int, int], triple2: Tuple[int, int, int], name: str = ''):
    ...
    def __get_default_constraints() -> Dict[Tuple[int, int, int], List[str]]:
        ...
        return {(row, column, value): [f"V{row}{column}", f"R{row}={value}",
                                       f"C{column}={value}", f"B{box(row, column)}={value}"]
