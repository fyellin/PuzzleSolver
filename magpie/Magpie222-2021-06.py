...
DOWNS = [
    ...
    (9, 2, generators.filterer(is_harshad)),
    (11, 2, generators.allvalues),
    (12, 3, generators.prime),  # also constraint below
...
def is_anagram(a: int | str, b: int | str):
    a, b = str(a), str(b)
    ...
    def get_clues() -> Sequence[Clue]:
        ...
        for information, is_across in ((ACROSSES, True), (DOWNS, False)):
            ...
            for number, length, generator in information:
                clue = Clue(f'{number}{letter}', is_across, grid[number - 1],
                            length, generator=generator)
                clues.append(clue)
        return clues

    def add_all_constraints(self) -> None:
        self.add_constraint(('12d',), lambda x: str(x)[0] == str(x)[2])
        ...
        self.add_constraint(("5d", "14a", "3d"),
                            lambda d5, a14, d3: is_anagram(int(d5) - int(a14), d3))
        self.add_constraint(("11d", "9d", "19a"),
                            lambda d11, d9, a19: int(d11) == int(d9) - int(a19))

    def check_clue(self, clue: Clue, known_clues: KnownClueDict) -> None:
        board = self.get_board(known_clues)
        return sum(int(x) for x in board.values()) == 126
    ...
    def draw_grid(self, **args: Unpack[DrawGridKwargs]) -> None:
        ...
        if True:
            ...
            args['top_bars'] = {(row, column) for row in range(2, 7) for column in range(1, 7)
                                if colors[row, column] != colors.get((row - 1, column))
                                }
            args['left_bars'] = {(row, column) for row in range(1, 7) for column in range(2, 7)
                                 if colors[row, column] != colors.get((row, column - 1))
                                 }
