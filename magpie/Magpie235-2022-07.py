...
import numpy as np
from more_itertools import first, sieve
...
def get_equations():
    ...
    return [word[-3:] for word in temp]
...
class MagpieSolver235Values(EquationSolver):
    ...
    def __init__(self):
        ...
        super().__init__(clues, items=islice(sieve(100), 13))
    ...
    def show_solution(self, known_clues: KnownClueDict, known_letters: KnownLetterDict):
        ...
        x = [first(int(clue.evaluators[0](known_letters)))for clue in self.clue_list]
        print(x)
    ...
    def build_entry_table(entries):
        ...
        for entry in entries:
            for base in range(2, 11):
                value = np.base_repr(entry, base=base)
