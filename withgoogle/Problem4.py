...
def goes_forever(a, b):
    if a == b:
        return False
    elif a > b:
        a, b = b, a

    if (a, b) in goes_forever_cache:
        return goes_forever_cache[a, b]

    while True:
        if (a ^ b) & 1 == 1:
            goes_forever_cache[a, b] = True
            return True
        if a & 1:
            break
        a >>= 1
        b >>= 1
    result = goes_forever_cache[a, b] = goes_forever(a * 2, b - a)
    return result

def solution(bananas):
    length = len(bananas)
    edges = [(i, j) for i, j in product(range(length), repeat=2) if goes_forever(bananas[i], bananas[j])]
    match = Match.from_edges(length, edges)
...
class SuperNode(Node):
    ...
    def circle(self, node):
        ...
        if i > 0 and self.subnodes[i].mate == self.subnodes[i - 1] or i == 0 and \
                self.subnodes[i].mate == self.subnodes[-1]:
