...
type Entry = str | int
type Constraint = str | tuple[str, str]
...
class FillInCrosswordGridAbstract(ABC):
    ...
    constraints: dict[Hashable, list[Constraint]]
    optional_constraints: set[str]
    finder: dict[tuple[int, Direction], dict[Square, list[list[Constraint]]]]
    ...
    def run(self, *, debug: int =0, square_type: SquareType = SquareType.FILLED
    ...
    def _solve(self, debug: int) -> list[Any]:
        ...
        def print_me(solution: Sequence[Hashable]) -> None:
            result: list[Clue] = [
                clue for constraint in solution
    ...
    def verify(self) -> bool:
        ...
        for constraint, values in self.constraints.items():
            if len(values) != len(set(values)):
                ...
                print(f"Constraint {constraint} has duplicates: {', '.join(duplicates)}")
        return ok
    ...
    def is_legal_pair_index(self, number1:int, location1: Square,
                            number2: int, location2: Square) -> bool:
    ...
    def handle_square_filling(
            self,
            square_type: SquareType | Callable[[Square], SquareType]) -> None:
        """
        ...
        get_square_type = ((lambda _ : square_type) if isinstance(square_type, SquareType)
                           else square_type)
    ...
    @classmethod
    @cache
    def _get_clue(cls, number: int, direction: Direction, location: Square, entry: Entry
                  ) -> MyClue:
...
class FillInCrosswordGrid (FillInCrosswordGridAbstract):
    ...
    def __init__(self, acrosses: Sequence[tuple[int, Entry]],
                 downs: Sequence[tuple[int, Entry]],
...
class FillInCrosswordGrid4Way (FillInCrosswordGrid):
    ...
    def __init__(self, acrosses: Sequence[int | str], downs: Sequence[int | str],
        ...
        assert Counter(across_length) == Counter(down_length), \
                ...
                if self.is_legal_pair_index(number3, location3, number4, location4):
                    super()._generate_constraint(
                        *clue_location_entry,
                        ...
                        (number4, Direction.DOWN, location4, value4), extras=extras)
...
class FillInCrosswordGridMushed (FillInCrosswordGridAbstract):
    ...
    def handle_odd_size_counts(self, clues_by_size: dict[int, list[tuple[int, str]]]
        ...
        def create_centered_entries(
                size: int, direction: Direction, extras: Sequence[str] = ()) -> int:
            ...
            for number, entry in clues_by_size[size]:
                self._generate_constraint((number, direction, location, entry),
                                          extras=extras)
...
class TestCrosswordGrids:
    ...
    def test_90_symmetric_grid(self) -> None:
        across_lengths: Sequence[Entry] = [6, "abcdefg", 4, 9, 10, 7, 4,
                          "qrstuv4", 5, 7, 5, 7, 5, 7, 4, 7, 10, 9, 4, 7, 6]
        down_lengths: Sequence[Entry] = [7, 9, 7, 7, 5, 4, "e123456789", 4, 6, 7,
