...
from solver import Clue, DancingLinks, DLConstraint, EquationSolver, Orderer
...
class FillInCrosswordGridAbstract(ABC):
    ...
    def _solve(self, debug: int) -> list[Sequence[DLConstraint]]:
    ...
    def handle_square_filling(
        ...
        for row in range(1, self.height + 1):
            for column in range(1, self.width + 1):
                t = f"r{row}c{column}"
                square_type = get_square_type((row, column))
                match square_type:
    ...
    def _generate_constraint(
        ...
        *clue_location_entries: tuple[int, Direction, Square, Entry],
        ...
        clues = tuple(starmap(self._get_clue, clue_location_entries))
        ...
        for clue in clues:
            if clue.context:
                info.extend(clue.dancing_links_rc_constraints(clue.context))
        # Indicate that we've used the clue. Each clue needs to be used once.
        info.extend(
            f"Clue-{clue.context or clue.name}" for clue in clues
        )
        info.extend(extras)
...
class FillInCrosswordGrid4Way(FillInCrosswordGrid):
    ...
    def __init__(
        self, acrosses: Sequence[Entry], downs: Sequence[Entry], *, width: int
    ):
