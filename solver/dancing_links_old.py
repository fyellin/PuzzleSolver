...
class DancingLinks(Generic[Row, Constraint]):
    ...
    def __solve_constraints_iterative_old(self) -> int:
        # Note that "depth" is meaningful only when debugging.
    ...
    def __solve_constraints_iterative(self) -> int:
        solution_count = 0
        stack: list[tuple[Literal[0], int] |
                    tuple[Literal[1], int, int, Row, set[Row], Iterator[tuple[int, Row]]] |
                    tuple[Literal[2], Sequence[tuple[Row, set[Row]]], Row]] = [(0, 0)]
            ...
            match frame:
                case (0, depth):
                    try:
                        ...
                        solution = [item[2] for item in stack if item[0] == 2]
                        if depth < self.max_debugging_depth:
                            self.output.write(f"{self.__indent(depth)}✓ SOLUTION\n")
                        solution_count += 1
                        self.row_printer(solution)
                    ...
                    rows = self.__cover_constraint(min_constraint)
                    stack.append((1, old_depth, new_depth, min_constraint, rows, iter(enumerate(rows, start=1))))

                case (1, old_depth, new_depth, min_constraint, rows, rows_iterator):
                    try:
                        index, row = next(rows_iterator)
                    except StopIteration:
                        self.__uncover_constraint(min_constraint, rows)
                        continue
                    stack.append(frame)  # repush this frame
                    cleanups = [(row_constraint, self.__cover_constraint(row_constraint))
                                for row_constraint in self.row_to_constraints[row]
                                if row_constraint != min_constraint]
                    if old_depth < self.max_debugging_depth:
                        self.__print_debug_info(min_constraint, row, index, len(rows), old_depth)
                    stack.append((2, cleanups, row))
                    stack.append((0, new_depth))

                case (2, cleanups, _row): # row here for generating the solution
                    for constraint, rows in reversed(cleanups):
                        self.__uncover_constraint(constraint, rows)
    ...
    def __cover_constraint(self, constraint: Constraint) -> set[Row]:
        ...
        for row in rows:
            ...
            for row_constraint in self.row_to_constraints[row]:
                ...
                if row_constraint != constraint:
                    if self.colors.get((row, row_constraint)) is not self.token:
    ...
    def __commit_constraint(self, row: Row, constraint: Constraint):
        if color := self.colors.get((row, constraint)) is None:
            self.__cover_constraint(constraint)
        else:
            self.__purify(constraint, color)

    def __uncommit_constraint(self, row: Row, constraint: Constraint):
        if color := self.colors.get((row, constraint)) is None:
            self.__uncover_constraint(constraint)
        else:
            self.__unpurify(constraint, color)
